...
> ⚠️ **Stability notice.** The deployed app is not fully optimised — known issues are catalogued in [`qa/stress_test_2026-04-26.md`](qa/stress_test_2026-04-26.md) and addressed in section 6. **PDF uploads above ~1 MB are slow** because no client-side PDF preprocessing was implemented — please use a `.txt` file or a lightweight PDF for best results. Errors may occasionally appear during generation; if that happens please **retry once** (the second attempt usually completes). If the second retry also fails, please [open an issue on GitHub](https://github.com/JR-00X/oxai-ai-studio-meeting-prep/issues/new?title=Cloud+Run+deployment+issue).
...
V2 added grounding rules (citation format, contradictions as first-class risks), counterparty-POV framing, and owner specificity. It closed citation hygiene (7/7 evidence fields with correct locators) and shifted talking points to named-stakeholder framing. But it also caught only one of three planted contradictions, surfaced the CTO-roadmap landmine *as a talking point to present to the CTO* (the inverse of what was needed), and introduced a deadline-fabrication regression — three invented dates appearing where the source had none.
...
A methods finding: the 9,501 → 6,478 token delta on an identical V3 prompt suggests branched chats leak context, so fresh chats are the more defensible protocol for clean model comparisons.
...
The live Cloud Run deployment was stress-tested independently ([`qa/stress_test_2026-04-26.md`](qa/stress_test_2026-04-26.md); Build-mode session in [`qa/ai_studio_build_log.md`](qa/ai_studio_build_log.md)). Core generation works (~24s for a small upload) and prompt-injection resistance held. Remaining issues form the productionisation roadmap. When an earlier round of the same test was handed to Build-mode, it applied the lower-risk fixes in-loop and self-bounded on the architecture rewrite — the same item topping this roadmap.

Highest-value bug fixes: enforce the 15 MB upload limit at the boundary; reject empty or whitespace-only files before generation; clear stale invalid files from the selected list; resolve the cover-art "Generating image..." status. Polish: `og:title` for sharing, and 44 px mobile hit targets.

Architecturally, the bigger investments: move the Gemini calls behind a proper backend on owned servers with a typed API tier (the browser-side `generateContent` is what Build-mode self-bounded on); add a server-side data-preprocessing pipeline — PDF text extraction, chunking, token-budget checks — so large decks become tractable rather than slow base64 payloads (the >1 MB PDF slowness is the symptom); upgrade image generation from the current free Pollinations proxy to a real image API; stream partial briefing output to improve perceived speed; route models (Flash by default, Pro on flagged deals); add explicit stakeholder-alignment examples in the prompt's contradiction rules so the model stops treating the pre-mortem as a second adversarial pass.
